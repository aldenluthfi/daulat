...
///
static Square battle_square_arg(int argc, char **argv, const char *key_x,
                                const char *key_y) {
  return (Square){
...
static void handle_battle(EngineState *engine, ...) {
  ...
  if (strcmp(argv[0], "moves") == 0 || strcmp(argv[0], "attacks") == 0) {
    ...
    emit_squares(strcmp(argv[0], "moves") == 0 ? battle_moves(battle, piece)
...
static void handle_campaign(EngineState *engine, ...) {
  ...
  if (strcmp(argv[0], "enter") == 0) {
    KingdomID kingdom = (KingdomID)arg_long(argc, argv, "id", KINGDOM_COUNT);
...
static void handle_map(EngineState *engine, ...) {
  ...
  if (strcmp(argv[0], "choose") == 0) {
    EventChoice choice = argc > 1 && argv[1][0] == 'b' ? CHOICE_B : CHOICE_A;
...
static void handle_codex(EngineState *engine, ...) {
  ...
  if (strcmp(argv[0], "pieces") == 0) {
    for (PieceID id = 0; id < PIECE_COUNT; id++) {
      if (PIECE_REGISTRY[id]) {
        protocol_emit("piece id=%d name=\"%s\"", id, PIECE_REGISTRY[id]->name);
      }
    }
  ...
  if (strcmp(argv[0], "cards") == 0) {
    for (CardID id = 0; id < CARD_COUNT; id++) {
      if (CARD_REGISTRY[id]) {
        protocol_emit("card id=%d name=\"%s\"", id, CARD_REGISTRY[id]->name);
      }
    }
  ...
  if (strcmp(argv[0], "relics") == 0) {
    for (RelicID id = 0; id < RELIC_COUNT; id++) {
      if (RELIC_REGISTRY[id].name) {
        protocol_emit("relic id=%d name=\"%s\"", id, RELIC_REGISTRY[id].name);
      }
    }
  ...
  if (strcmp(argv[0], "info") == 0) {
    ...
    if (piece >= 0 && piece < PIECE_COUNT && PIECE_REGISTRY[piece]) {
      protocol_emit("info name=\"%s\" desc=\"%s\"", PIECE_REGISTRY[piece]->name,
                    PIECE_REGISTRY[piece]->desc);
    } else if (card >= 0 && card < CARD_COUNT && CARD_REGISTRY[card]) {
      protocol_emit("info name=\"%s\" desc=\"%s\"", CARD_REGISTRY[card]->name,
...
static void handle_settings(EngineState *engine, ...) {
  ...
  if (strcmp(argv[0], "show") == 0) {
    protocol_emit("settings cleared=%d masteries=%d,%d,%d,%d,%d",
                  engine->cleared, engine->masteries[0], engine->masteries[1],
                  engine->masteries[2], engine->masteries[3],
                  engine->masteries[4]);
    protocol_emit("ok");
    return;
  }
