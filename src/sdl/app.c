...
void app_consume_line(App* app, const char* line) {
    ...
    if (strcmp(parsed.verb, "SHOW") == 0) {
        ...
        while (*tail != '\0' && isspace((unsigned char)*tail))
            tail++;
        snprintf(
            app->model.popup_detail, sizeof(app->model.popup_detail),
            "%s", tail
        );
...
static void drain_engine(App* app) {
    ...
    while (true) {
        size_t count = fread(
            chunk, 1, sizeof(chunk), app->engine_out
        );
        if (count == 0)
            break;
...
static void render_model(App* app) {
    ...
    SDL_SetRenderDrawColor(
        app->renderer, style.fill.r, style.fill.g, style.fill.b, 255
    );
    ...
    if (strcmp(app->model.screen, "title") == 0) {
        ui_text(app->renderer, 32.0f, 96.0f,
    ...
    if (app->model.popup[0] != '\0') {
        char popup[128];
        snprintf(
            popup, sizeof(popup), "POPUP %s %s",
            app->model.popup, app->model.popup_detail
        );
...
SDL_AppResult app_event(App* app, const SDL_Event* event) {
    ...
    if (strcmp(app->model.screen, "title") == 0) {
        switch (key) {
            case SDL_SCANCODE_1: app_send(app, "new_run");          break;
            case SDL_SCANCODE_2: app_send(app, "load_run");         break;
            case SDL_SCANCODE_3: app_send(app, "open_codex piece"); break;
            case SDL_SCANCODE_4: app_send(app, "goto mastery");     break;
            case SDL_SCANCODE_5: app_send(app, "goto settings");    break;
            default:                                                break;
        }
    }
    return SDL_APP_CONTINUE;
