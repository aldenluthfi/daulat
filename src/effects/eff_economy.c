...
void eff_cp_add(struct EffectCtx* context, const EffectArg* args, size_t count) {
...
void eff_cp_set(struct EffectCtx* context, const EffectArg* args, size_t count) {
    struct BattleState* battle = (struct BattleState*)context;
    if (count < 1)
        return;
...
void eff_cost_mod(struct EffectCtx* context, const EffectArg* args, size_t count) {
    (void)context;
    (void)args;
    (void)count;
}
...
void eff_sell_mod(struct EffectCtx* context, const EffectArg* args, size_t count) {
    (void)context;
    (void)args;
    (void)count;
}
...
void eff_income_add(struct EffectCtx* context, const EffectArg* args, size_t count) {
    (void)context;
    (void)args;
    (void)count;
}
